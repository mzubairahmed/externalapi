...
public final class ProductParserUtil {
	...
	public final static String getConfigId(
	...
	public static ProductInventoryLink getInventoryLink(String inventoryLink,
			ProductDetail existingProduct, String companyId) {
		ProductInventoryLink productInventoryLink = null;
		if (existingProduct != null) {
			productInventoryLink = existingProduct.getProductInventoryLink() != null ? existingProduct
					.getProductInventoryLink() : new ProductInventoryLink();
			productInventoryLink.setUrl(inventoryLink);
			productInventoryLink.setProductId(String.valueOf(existingProduct
					.getID()));
	...
	public static ProductDataSheet getDataSheet(String productDataSheet,
			ProductDetail existingProduct, String companyId) {
		ProductDataSheet prodDataSheet = new ProductDataSheet();
		if (existingProduct != null) {
			prodDataSheet = existingProduct.getProductDataSheet() != null ? existingProduct
					.getProductDataSheet() : new ProductDataSheet();
	...
	public static Integer getCriteriaSetValueId(String xid,
			String criteriaCode, Object value) {
		...
		if (null != value) {
			...
			if (null != listObj && listObj.size() > 0) {
				for(Object crntObj: listObj){
				if (crntObj instanceof String) {  // String
					...
					if(listObj.size()==1){
					result = ProductDataStore.findCriteriaSetValueIdForValue(
							xid, criteriaCode, ((Value) crntObj).getValue() + ":"
									+ ((Value) crntObj).getUnit());
					}else{		
						listofValue=true;
						if(null==srcCriteriaValue){
						srcCriteriaValue=((Value) crntObj).getAttribute().toUpperCase() + ":"+((Value) crntObj).getValue() + ":"
								+ ((Value) crntObj).getUnit();
						}else{
							srcCriteriaValue+=";"+((Value) crntObj).getAttribute().toUpperCase() + ":"+((Value) crntObj).getValue() + ":"
									+ ((Value) crntObj).getUnit();
		...
		if(listofValue){
			result = ProductDataStore.findCriteriaSetValueIdForValue(
					xid, criteriaCode, srcCriteriaValue);
		}
		if (result != null) {
	...
	public static String getCriteriaCodeFromCriteria(String criteria, String xid) {
		...
		CriteriaInfo criteriaInfo = ProductDataStore
				.getCriteriaInfoByDescription(criteria, xid);
	...
	public static String getCriteriaNameFromCriteriaCode(String criteria) {
		...
		CriteriaInfo criteriaInfo = ProductDataStore
				.getCriteriaInfoForCriteriaCode(criteria);
		return criteria != null ? criteriaInfo.getDescription() : null;
	}

	public static String getSizeValuesFromSize(Size size,
			String sizeCriteriaCode) {
		if (sizeCriteriaCode
				.equalsIgnoreCase(ApplicationConstants.CONST_SIZE_GROUP_DIMENSION)) {
			return getDimensionValues(size.getDimension());
		} else if (sizeCriteriaCode
		...
		for (Values individualDim : dims.getValues()) {
			...
			for (Value v : individualDim.getValue()) {
				if (!individualSize.isEmpty()) {
					individualSize = CommonUtilities.appendValue(
							individualSize, "", ";");
				}
			...
			if (!dimValues.isEmpty()) {
				dimValues = CommonUtilities.appendValue(dimValues,
						individualSize, ",");
			} else {
	...
	private static String getVolumeValues(Volume vols) {
		...
		for (Values individualVol : vols.getValues()) {
			...
			if (!volumeValues.isEmpty()) {
				volumeValues = CommonUtilities.appendValue(volumeValues,
						individualSize, ",");
			} else {
	...
	private static String getCSVSizesFromSizeModel(List<Value> values) {
		...
		for (Value v : values) {
			if (finalValues.isEmpty()) {
				...
				finalValues = CommonUtilities.appendValue(finalValues,
						v.getValue(), ",");
			}
		}
	...
	public static String getShippingDimension(ShippingEstimate shippingEstimate) {
		...
		if (shippingEstimate != null
				&& shippingEstimate.getDimensions() != null) {
			shippingDimension = CommonUtilities.appendValue(shippingDimension,
					shippingEstimate.getDimensions().getLength(), "");
			shippingDimension = CommonUtilities.appendValue(shippingDimension,
					shippingEstimate.getDimensions().getLengthUnit(), ":");
			shippingDimension = CommonUtilities.appendValue(shippingDimension,
					shippingEstimate.getDimensions().getWidth(), ";");
			shippingDimension = CommonUtilities.appendValue(shippingDimension,
					shippingEstimate.getDimensions().getWidthUnit(), ":");
			shippingDimension = CommonUtilities.appendValue(shippingDimension,
					shippingEstimate.getDimensions().getHeight(), ";");
			shippingDimension = CommonUtilities.appendValue(shippingDimension,
					shippingEstimate.getDimensions().getHeightUnit(), ":");
	...
	public static String getShippingItem(ShippingEstimate shippingEstimate) {
		...
		if (shippingEstimate != null
				&& shippingEstimate.getNumberOfItems() != null) {
			numberOfItems = CommonUtilities.appendValue(numberOfItems,
					((Value) shippingEstimate.getNumberOfItems()).getValue(),
					"");
			numberOfItems = CommonUtilities.appendValue(numberOfItems,
					((Value) shippingEstimate.getNumberOfItems()).getUnit(),
					":");
	...
	public static boolean isValidAvailabilty(Availability availability,
			boolean isOptionGroup) {

		if (availability != null) {
			// Check both criteria present or not
			if (!CommonUtilities.isValueNull(availability.getParentCriteria())
					&& !CommonUtilities.isValueNull(availability
							.getChildCriteria())) {
				// now check the equality of Criteria
				if (availability.getParentCriteria().trim()
						.equalsIgnoreCase(availability.getChildCriteria())) {
					// The only case criteria code can be equal is for Options,
					// but then its OptionName cannot be equal
					if (!CommonUtilities.isValueNull(availability
							.getParentOptionName())
							&& !CommonUtilities.isValueNull(availability
									.getChildOptionName())) {
						if (!availability
								...
								.trim()
								.equalsIgnoreCase(
										availability.getChildOptionName()
												.trim())) {
							return true;
						} else {
							return false;
	...
	public static ProductCriteriaSets getOptionCriteriaSet(String code,
			String optName,
		...
		if (criteriaSets != null) {
			...
			for (ProductCriteriaSets criteriaSet : criteriaSets) {
				if (!checkOptionName) {
					if (String.valueOf(criteriaSet.getCriteriaDetail())
							.equalsIgnoreCase(optName)) {
						return criteriaSet;
					}
				} else {
					return criteriaSet;
	...
	public static String getRelationNameBasedOnCodes(String parent,
			String child, Availability availability) {
		String name = getCriteriaNameFromCriteriaCode(parent);
		if (CommonUtilities.isValueNull(name)) {
			name = availability.getParentCriteria();
		}
		if (CommonUtilities.isOptionGroup(parent)
				&& !CommonUtilities.isValueNull(availability
						.getParentOptionName())) {
		...
		if (CommonUtilities.isOptionGroup(child)
				&& !CommonUtilities.isValueNull(availability
						.getChildOptionName())) {
	...
	public static boolean isSizeGroup(String criteriaCode) {
		return ApplicationConstants.SIZE_GROUP_CRITERIACODES
				.contains(criteriaCode);
	}
	...
	public static String getCriteriaSetValueIdBaseOnValueType(String xid,
		...
		if (value.get(0) instanceof String) {
			...
			if (null != valueToSearch && valueToSearch.contains("=")) {
				valueToSearch = valueToSearch.substring(valueToSearch
						.indexOf("=") + 1);
				if (valueToSearch.endsWith("}"))
					valueToSearch = valueToSearch.substring(0,
	...
	private static String getSizeModelFromObject(String criteriaCode,
			Object value) {
		...
		if (criteriaCode
				...
				if (value instanceof List) {
					...
					for (Map<?, ?> dimValue : dimValuesList) {
						if (dimValue.get("Attribute") != null) {
							finalValue = CommonUtilities.appendValue(
									finalValue, dimValue.get("Attribute") + ":"
											+ dimValue.get("Value") + ":"
											+ dimValue.get("Unit"), ";");
						}
					}
					...
					for (Value dimsValue : dimsValues) {
						finalValue += dimsValue.getAttribute() + ":"
								+ dimsValue.getValue() + ":"
								+ dimsValue.getUnit() + ";";
					}
					finalValue = finalValue.substring(0,
							finalValue.lastIndexOf(";"));
			...
			try {
				if (value instanceof List) {
					...
					return String.valueOf(((Value) value).getValue() + ":"
							+ (((Value) value).getUnit()));
				} else {
			...
			try {
				if (value instanceof List) {
					...
					if (volumes != null && !volumes.isEmpty()) {
						List<Map<?, ?>> values = (List<Map<?, ?>>) volumes
								.get(0);
						for (Map<?, ?> v : values) {
							valueToSearch = v.get("Value") + ":"
									+ v.get("Unit");
						}
						return valueToSearch;
					}
			...
			try {
				...
				|| criteriaCode.equalsIgnoreCase("SAIT")
				|| criteriaCode.equalsIgnoreCase("SANS")
				|| criteriaCode.equalsIgnoreCase("SAWI")
	...
	public boolean isProductHasValidProductNumber(Product serProduct) {
		...
		if (serProduct.getPriceGrids() != null
				&& !serProduct.getPriceGrids().isEmpty()) {
			for (PriceGrid pg : serProduct.getPriceGrids()) {
				if (pg != null
						&& !CommonUtilities.isValueNull(pg.getProductNumber())) {
					pnoFound = true;
					break;
				}
			}
		}
		if (serProduct.getProductNumbers() != null
				&& !serProduct.getProductNumbers().isEmpty()) {
