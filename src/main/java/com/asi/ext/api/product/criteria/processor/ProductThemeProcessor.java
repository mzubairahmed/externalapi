...
public class ProductThemeProcessor extends SimpleCriteriaProcessor {
	
    private int                 uniqueCriteriaSetId = 1;
    private String              configId            = "0";

    private final static Logger LOGGER              = Logger.getLogger(ProductThemeProcessor.class.getName());
     ...
     * @param configId
     */
	public ProductThemeProcessor(int uniqueCriteriaSetId, String configId) {
        this.uniqueCriteriaSetId = uniqueCriteriaSetId;
        this.configId = configId;
	}
	
    public ProductCriteriaSets getThemeCriteriaSet(List<String> themes, ProductDetail rdrProduct, ProductCriteriaSets productCriteriaSets, String configId) {
        this.configId = configId;
        return getCriteriaSet(CommonUtilities.convertStringListToCSV(themes), rdrProduct, productCriteriaSets, 0);
    }


	@Override
	protected ProductCriteriaSets getCriteriaSet(String values, ProductDetail existingProduct, ProductCriteriaSets matchedCriteriaSet, int currentSetValueId) {
        if (!updateNeeded(matchedCriteriaSet, values)) {
            return null;
        }

        LOGGER.info("Started Processing of Product Themes");
        ...
        if (checkExistingElements) {
            ...
            matchedCriteriaSet.setCriteriaCode(ApplicationConstants.CONST_THEME_CRITERIA_CODE);
        ...
        for (String value : finalValues) {
            ...
            if (criteriaSetValue == null) {
                ...
                criteriaSetValue.setCriteriaCode(ApplicationConstants.CONST_THEME_CRITERIA_CODE);
            ...
            updateReferenceTable(existingProduct.getExternalProductId(), ApplicationConstants.CONST_THEME_CRITERIA_CODE, value, criteriaSetValue);
        ...
        LOGGER.info("Completed Processing of Product Themes");
	...
	public String getSetCodeValueId(String value) {
        return ProductDataStore.getSetCodeValueIdForProductTheme(value.trim());
	}
    ...
    public boolean registerExistingValuesForReference(ProductCriteriaSets criteriaSet, String externalProductId) {
        ...
        LOGGER.info("Registering existing product Theme values");
        if (criteriaSet.getCriteriaSetValues() != null && !criteriaSet.getCriteriaSetValues().isEmpty()) {
            for (CriteriaSetValues criteriaValues : criteriaSet.getCriteriaSetValues()) {
                if (criteriaValues.getCriteriaSetCodeValues().length != 0) {
                  ...
                  updateReferenceTable(externalProductId, ApplicationConstants.CONST_THEME_CRITERIA_CODE, valueToRegister, criteriaValues);
                }
            }
        }
        LOGGER.info("Completed registering existing product Theme values");
