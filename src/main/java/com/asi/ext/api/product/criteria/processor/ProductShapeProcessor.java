...
public class ProductShapeProcessor extends SimpleCriteriaProcessor {
    ...
    public ProductCriteriaSets getShapeCriteriaSet(List<String> shapes, ProductDetail rdrProduct,
            ProductCriteriaSets productCriteriaSets, String configId) {
        this.configId = configId;
        return getCriteriaSet(CommonUtilities.convertStringListToCSV(shapes), rdrProduct, productCriteriaSets, 0);
    }
        ...
        if (criteriaSet.getCriteriaSetValues() != null && !criteriaSet.getCriteriaSetValues().isEmpty()) {
            for (CriteriaSetValues criteriaValues : criteriaSet.getCriteriaSetValues()) {
                if (criteriaValues.getCriteriaSetCodeValues().length != 0) {
                  ...
                  updateReferenceTable(externalProductId, ApplicationConstants.CONST_SHAPE_CRITERIA_CODE, valueToRegister, criteriaValues);
                }
            }
        }
