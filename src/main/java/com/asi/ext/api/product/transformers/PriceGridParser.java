...
public class PriceGridParser extends ProductParser {
	...
	private ArrayList<String> SIZE_GROUP_CRITERIACODES = new ArrayList<String>(
			Arrays.asList("SABR", "SAHU", "SAIT", "SANS", "SAWI", "SSNM",
					"SVWT", "DIMS", "CAPS", "SOTH"));
	...
	private Set<String> getPriceCriteriaCodes(
			List<PriceConfiguration> priceConfigs, String xid) {
		Set<String> criteriaCodes = new HashSet<String>();
		for (PriceConfiguration config : priceConfigs) {
			CriteriaInfo crtInfo = ProductDataStore
					.getCriteriaInfoByDescription(config.getCriteria(), xid);
			if (crtInfo != null) {
	...
	private String getPriceGridSubTypeCode(boolean priceType,
			List<PriceConfiguration> priceConfigs, String xid) {
		if (priceType) { // Base price
			...
			if (criteriaCodes.size() > 1) {
				...
				return PriceGridUtil.getPriceGridSubTypeCode(
						CommonUtilities.getElementFromSet(criteriaCodes, 1),
						priceType);
			} else {
				return ApplicationConstants.CONST_UPCHARGE_PRICE_GRID_CODE;
			}
	...
	private PriceUnit getPriceUnit(
			com.asi.ext.api.service.model.PriceUnit serPunit) {
		return ProductDataStore.getPriceUnit(serPunit.getName());
	}

	private String getPriceUnitName(
			com.asi.ext.api.service.model.PriceUnit serPunit) {
	...
	private List<com.asi.service.product.client.vo.Price> getPrices(
			List<Price> prices, String pGridId) {
		...
		for (Price serPrice : prices) {
			...
			if (serPrice.getPriceUnit() != null
					&& serPrice.getPriceUnit().getItemsPerUnit() != null) {
				veloPrice
						.setItemsPerUnit(serPrice.getPriceUnit() != null
								&& serPrice.getPriceUnit().getItemsPerUnit() != null ? Integer
								.parseInt(serPrice.getPriceUnit()
										.getItemsPerUnit()) : 1);
			} else {
			...
			veloPrice
					.setPriceUnitName(getPriceUnitName(serPrice.getPriceUnit()));
	...
	private PriceGrid getMatchingPriceGrid(List<PriceGrid> extPriceGrids,
			com.asi.ext.api.service.model.PriceGrid serPGrid) {
		boolean needDeepComapre = false;
		String extPriceModelString = getPriceModelStringFromServiceModel(
				serPGrid, serPGrid.getPrices(), needDeepComapre);
		PriceGrid firstMatch = null;
		for (PriceGrid pGrid : extPriceGrids) {
			if (serPGrid.getIsBasePrice().equals(pGrid.getIsBasePrice())) {
				String extPgString = getPriceModelString(pGrid,
						pGrid.getPrices(), false);
		...
		if (needDeepComapre) {
			extPriceModelString = getPriceModelStringFromServiceModel(serPGrid,
					serPGrid.getPrices(), needDeepComapre);
			for (PriceGrid pGrid : extPriceGrids) {
				if (serPGrid.getIsBasePrice().equals(pGrid.getIsBasePrice())) {
					String extPgString = getPriceModelString(pGrid,
							pGrid.getPrices(), needDeepComapre);
					if (extPriceModelString.equalsIgnoreCase(extPgString)) {
						return pGrid;
					}
				}
	...
	private String getPriceModelString(PriceGrid pGrid,
			List<com.asi.service.product.client.vo.Price> prices,
			boolean deepComparison) {
		String pString = pGrid.getIsBasePrice() + "###" + pGrid.getIsQUR();
		if (deepComparison) {
			pString = pGrid.getIsBasePrice() + "###"
					+ pGrid.getDisplaySequence() + "###" + pGrid.getIsQUR()
		...
		for (com.asi.service.product.client.vo.Price p : prices) {
			...
			pString = CommonUtilities.appendValue(pString,
					String.valueOf(p.getListPrice()), "$$$");
			pString = CommonUtilities.appendValue(pString, String.valueOf(p
	...
	private String getPriceModelStringFromServiceModel(
			com.asi.ext.api.service.model.PriceGrid pGrid, List<Price> prices,
			boolean deepComparison) {
		String pString = pGrid.getIsBasePrice() + "###" + pGrid.getIsQUR();
		if (deepComparison) {
			pString = pGrid.getIsBasePrice() + "###" + pGrid.getSequence()
					+ "###" + pGrid.getIsQUR() + "###"
					+ pGrid.getPriceIncludes();
		}
		for (Price p : prices) {
			...
			pString = CommonUtilities.appendValue(pString,
					String.valueOf(p.getListPrice()), "$$$");
			pString = CommonUtilities.appendValue(pString,
					String.valueOf(p.getDiscountCode()), "$$$");
			// final delim
			pString = CommonUtilities.appendValue(pString, "---", "");
		}
	...
	public List<PriceGrid> getPriceGrids(
			List<com.asi.ext.api.service.model.PriceGrid> servicePriceGrids,
			ProductDetail product) {
		int priceGridId = -1;
		uniqPItemId = -1;

		List<ProductNumber> productNumbers = new ArrayList<ProductNumber>();
		Map<String, ProductNumber> extPnoMap = productNumberProcessor
				.getExistingProductNumberMapForPriceGrid(product
						.getProductNumbers());
		...
		for (com.asi.ext.api.service.model.PriceGrid serPGrid : servicePriceGrids) {
			...
			hasProductNumber = !CommonUtilities.isValueNull(serPGrid
					.getProductNumber());
			...
			newPGrid.setUsageLevelCode(getUsageLevelCode(serPGrid
					.getIsBasePrice()));
			newPGrid.setPriceGridSubTypeCode(getPriceGridSubTypeCode(
					serPGrid.getIsBasePrice(),
					serPGrid.getPriceConfigurations(),
					product.getExternalProductId()));
			PriceGrid extPriceGrid = getMatchingPriceGrid(
					product.getPriceGrids(), serPGrid);
			...
			Collections.sort(serPGrid.getPriceConfigurations(),
					new PriceCriteriaComparator());

			if (extPriceGrid != null && extPriceGrid.getPricingItems() != null) {
				newPGrid.setPricingItems(getPricingItems(
						serPGrid.getPriceConfigurations(), newPGrid.getID(),
						product.getID(), newPGrid.getIsBasePrice(),
						extPriceGrid.getPricingItems(),
						product.getExternalProductId()));
			} else {
				newPGrid.setPricingItems(getPricingItems(
						serPGrid.getPriceConfigurations(), newPGrid.getID(),
						product.getID(), newPGrid.getIsBasePrice(), null,
						product.getExternalProductId()));
			}

			if (hasProductNumber) {
				productNumbers.add(productNumberProcessor
						.getProductNumberForPriceGrid(newPGrid.getID(),
								serPGrid.getProductNumber(),
								newPGrid.getPricingItems(),
								extPnoMap.get(newPGrid.getID()),
								product.getID()));
			}
	...
	private List<PricingItem> getPricingItems(
			List<PriceConfiguration> priceConfigs, String pGridId,
			String productId, boolean isBasePrice, List<PricingItem> extPItem,
			String xid) {
		List<PricingItem> finalPricingItems = new ArrayList<PricingItem>();

		for (PriceConfiguration pConfig : priceConfigs) {
			CriteriaInfo criteriaInfo = ProductDataStore
					.getCriteriaInfoByDescription(pConfig.getCriteria(), xid);
			if (criteriaInfo == null) {
				// TODO: LOG Validation ERROR
				productDataStore.addErrorToBatchLogCollection(xid,
						ApplicationConstants.CONST_BATCH_ERR_INVALID_VALUE,
						"Criteria specified for product doesn't exist");
				continue;
			}

			String criteriaSetValueId = ProductParserUtil
					.getCriteriaSetValueIdBaseOnValueType(xid,
							criteriaInfo.getCode(), pConfig.getValue());
			if (criteriaSetValueId == null) {
				productDataStore.addErrorToBatchLogCollection(xid,
						ApplicationConstants.CONST_BATCH_ERR_INVALID_VALUE,
						"Criteria value specified for product doesn't exist, value : "
								+ pConfig.getValue());
			} else {
				finalPricingItems.add(getPricingItem(criteriaSetValueId,
						pGridId, productId, isBasePrice, extPItem));
			}
	...
	private PricingItem getPricingItem(String criteriaSetValueId,
			String pGridId, String productId, boolean isBasePrice,
			List<PricingItem> extPricingItems) {
		if (extPricingItems != null && !extPricingItems.isEmpty()) {
			for (PricingItem item : extPricingItems) {
				if (criteriaSetValueId.equalsIgnoreCase(item
		...
		return createNewPricingItem(criteriaSetValueId, pGridId, productId,
				isBasePrice);
	}

	private PricingItem createNewPricingItem(String crtValueId, String pGridId,
			String productId, boolean isBasePrice) {
	...
	protected ProductCriteriaSets getLessThanMinimumCriteriaSet(Product product)
			throws VelocityException {

		if (lessThanMinimumSetCodeValueMap == null
				|| lessThanMinimumSetCodeValueMap.isEmpty()) {
			@SuppressWarnings("static-access")
			String lessThanMinWSResponse = orgnCall
					.getLookupsResponse(RestAPIProperties
							.get(ApplicationConstants.LESS_THAN_MIN_LOOKUP));
			lessThanMinimumSetCodeValueMap = jsonProcessorObj
					.getSetCodeValuesForIndividualCriteriaCode(
							lessThanMinWSResponse,
		...
		productCriteriaSet
				.setCriteriaCode(ApplicationConstants.CONST_LESS_THAN_MIN_CRT_CODE);
		productCriteriaSet.setCompanyId(product.getProductConfigurations()[0]
				.getProductCriteriaSets()[0].getCompanyId());
		productCriteriaSet
				.setCriteriaSetId(String.valueOf(criteriaSetUniqueId));
		productCriteriaSet.setProductId(product.getProductConfigurations()[0]
				.getProductCriteriaSets()[0].getProductId());
		productCriteriaSet
				.setIsRequiredForOrder(ApplicationConstants.CONST_STRING_FALSE_SMALL);
		productCriteriaSet.setConfigId(product.getProductConfigurations()[0]
				.getProductCriteriaSets()[0].getConfigId());
		// Find from setCodeValues of LessThanMin JSON Dataset
		String setCodeValueId = lessThanMinimumSetCodeValueMap
				.get(ApplicationConstants.CONST_STRING_OTHER) != null ? lessThanMinimumSetCodeValueMap
		...
		if (setCodeValueId != null) {
			...
			criteriaSetValue.setCriteriaSetId(productCriteriaSet
					.getCriteriaSetId());
			criteriaSetValuesID = criteriaSetValuesID - 1;
			criteriaSetValue.setId(criteriaSetValuesID + "");
			criteriaSetValue
					.setCriteriaCode(ApplicationConstants.CONST_LESS_THAN_MIN_CRT_CODE);
			productDataStore.updateCriteriaSetValueReferenceTable(product
					.getExternalProductId().trim(),
					ApplicationConstants.CONST_LESS_THAN_MIN_CRT_CODE, "Y",
					criteriaSetValue.getId());

			criteriaSetValue.setCriteriaValueDetail(product
					.getProductConfigurations()[0].getProductCriteriaSets()[0]
					.getCriteriaSetValues()[0].getCriteriaValueDetail());
			criteriaSetValue.setIsSubset(product.getProductConfigurations()[0]
					.getProductCriteriaSets()[0].getCriteriaSetValues()[0]
					.getIsSubset());
			criteriaSetValue.setIsSetValueMeasurement(product
					.getProductConfigurations()[0].getProductCriteriaSets()[0]
			...
			criteriaSetCodeValue
					.setCriteriaSetValueId(criteriaSetValuesID + "");
			...
			productCriteriaSet
					.setCriteriaSetValues(new CriteriaSetValues[] { criteriaSetValue });
	...
	public synchronized List<PricingItems> generatePricingItemsForCriteria(
			String priceCriteria, Product product, PriceGrids priceGrids,
			String pricecriteriaCode, boolean isBasePrice)
			throws AmbiguousPriceCriteriaException {
		validCriteriaValues = "";
		try {
			if (!CommonUtilities.isValueNull(priceCriteria)) {
				if (CommonUtilities.isValidPriceCriteria(priceCriteria)) {
					String criteriaCode = priceCriteria.trim().substring(0,
							priceCriteria.indexOf(":"));
					if (isBasePrice
							&& !criteriaCode
									.equalsIgnoreCase(pricecriteriaCode)
							&& !relaxBasePriceRule(criteriaCode,
									pricecriteriaCode)) {
						throw new AmbiguousPriceCriteriaException(
								"Ambiguous/Invalid Price Criteria "
										+ CommonUtilities
												.getStringValue(criteriaCode)
										+ " specified for PriceGrid");
					}

					if (isBasePrice
							&& !isCriteriaCodeValidForPriceGrid(criteriaCode,
									isBasePrice)) {
						productDataStore
								.addErrorToBatchLogCollection(
					...
					if (!isValidOptionCriteria(
							priceCriteria.substring(criteriaCode.length() - 1),
							criteriaCode)) {
						return new ArrayList<>();
					}
					Set<String> criteriaValues = getBasePriceCriteriaValues(
							priceCriteria, criteriaCode);
					...
					if (criteriaValues != null && !criteriaValues.isEmpty()) {
						...
						for (String criteria : criteriaValues) {
							@SuppressWarnings("static-access")
							String criteriaSetValueId = productDataStore
									.findCriteriaSetValueIdForValue(product
											.getExternalProductId().trim(),
											criteriaCode, criteria);

							if (!CommonUtilities
									.isValueNull(criteriaSetValueId)) {
								PricingItems pricingItems = new PricingItems();
								if (validCriteriaValues.isEmpty()) {
									validCriteriaValues = criteria;
								} else {
									validCriteriaValues += ApplicationConstants.CONST_STRING_COMMA_SEP
											+ criteria;
								}
								pricingItems
										.setId(ApplicationConstants.CONST_STRING_ZERO);
								pricingItems
										.setCriteriaSetValueId(criteriaSetValueId);
								pricingItems
										.setId(ApplicationConstants.CONST_STRING_ZERO);
								pricingItems.setProductId(product.getId());
								pricingItems
										.setMarketSegmentCode(ApplicationConstants.CONST_MARKET_SEGMENT_CODE);
								pricingItems
										.setPriceGridTypeCode(ApplicationConstants.CONST_PRICE_GRID_ITEM_CODE);
								...
								productDataStore
										.addErrorToBatchLogCollection(
												...
												"PriceGrid - Given Criteria value "
														+ criteria
														+ " doesn't exist");
							}
						}
						...
						productDataStore
								.addErrorToBatchLogCollection(
										product.getExternalProductId().trim(),
										ApplicationConstants.CONST_BATCH_ERR_LOOKUP_VALUE_NOT_EXIST,
										"PriceGrid Criteria " + priceCriteria
												+ " is not valid");
					}
				} else {
					productDataStore.addErrorToBatchLogCollection(product
							.getExternalProductId().trim(),
							ApplicationConstants.CONST_BATCH_ERR_INVALID_VALUE,
							"Invalid Price Criteria " + priceCriteria);
					return new ArrayList<>();
				}
			} else {
				return new ArrayList<>();
	...
	private static Set<String> getBasePriceCriteriaValues(String source,
			String criteriaCode) {
		if (!CommonUtilities.isValueNull(source)) {
			...
			if (index != -1 && source.length() > index) {
				...
				if (criteriaCode
					...
					temp = CommonUtilities.getOriginalCSVValues(source
							.substring(index + 1, source.length()));
				} else {
					temp = source.substring(index + 1, source.length()).split(
	...
	public static String getValidPriceCriteriaCode(String[] priceCriterias,
			String[] priceCriterias2, boolean isBasePrice) {
		...
		if (priceCriterias != null && priceCriterias.length > 0) {
			...
			if (isBasePrice && !CommonUtilities.isValueNull(priceCriterias[0])
				...
				priceCriteriaCode = priceCriterias[0].trim().substring(0,
						priceCriterias[0].trim().indexOf(":"));
				if (PriceGridUtil.isCriteriaCodeValidForPriceGrid(
						priceCriteriaCode, isBasePrice)) {
					priceCriteriaCode += "#$"
							+ (priceCriterias2[0].trim().indexOf(":") != -1 ? priceCriterias2[0]
									.trim().substring(
											0,
											priceCriterias2[0].trim().indexOf(
													":")) : "null");

					return priceCriteriaCode;
				}
			} else if (!isBasePrice
					&& !CommonUtilities.isValueNull(priceCriterias[1])
					&& CommonUtilities.isValidPriceCriteria(priceCriterias[1])) {
				priceCriteriaCode = priceCriterias[1].trim().substring(0,
						priceCriterias[1].trim().indexOf(":"));
				if (PriceGridUtil.isCriteriaCodeValidForPriceGrid(
						priceCriteriaCode, isBasePrice)) {

					priceCriteriaCode += "#$"
							+ (priceCriterias2[1].trim().indexOf(":") != -1 ? priceCriterias2[1]
									.trim().substring(
											0,
											priceCriterias2[1].trim().indexOf(
													":")) : "null");

					return priceCriteriaCode;
				}
			}
			// If first set dosen't have a valid Criteria set so find next valid
			// one

			for (int i = 0; i < priceCriterias.length; i++) {

				if (((i % 2) == 0 && isBasePrice)
						&& !CommonUtilities.isValueNull(priceCriterias[i])
						&& CommonUtilities
								.isValidPriceCriteria(priceCriterias[i])) {
					priceCriteriaCode = priceCriterias[i].trim().substring(0,
							priceCriterias[i].trim().indexOf(":"));
					if (PriceGridUtil.isCriteriaCodeValidForPriceGrid(
							priceCriteriaCode, isBasePrice)) {

						priceCriteriaCode += "#$"
								+ (priceCriterias2[i].trim().indexOf(":") != -1 ? priceCriterias2[i]
										.trim().substring(
												0,
												priceCriterias2[i].trim()
														.indexOf(":")) : "null");

						return priceCriteriaCode;
					}
				} else if (((i % 2) != 0 && !isBasePrice)
						&& !CommonUtilities.isValueNull(priceCriterias[0])
						&& CommonUtilities
								.isValidPriceCriteria(priceCriterias[i])) {
					priceCriteriaCode = priceCriterias[i].trim().substring(0,
							priceCriterias[i].trim().indexOf(":"));
	...
	protected static String getPriceGridDescription(PriceGrids pGrids,
	...
	public static PriceGrids compareAndUpdatePriceGrid(
			PriceGrids newPriceGrids, PriceGrids existingPriceGrid,
	...
	public static boolean isValidOptionCriteria(String priceCriteria,
			String criteriaCode) {
		if (criteriaCode
				.equalsIgnoreCase(ApplicationConstants.CONST_PRODUCT_OPTION)
				|| criteriaCode
						.equalsIgnoreCase(ApplicationConstants.CONST_SHIPPING_OPTION)
				|| criteriaCode
						.equalsIgnoreCase(ApplicationConstants.CONST_IMPRINT_OPTION)) {
			return true;
		}
		return true;
	}
	...
	public static boolean isValidPriceSet(String price, String quantity) {
		...
		if (CommonUtilities.isValueNull(price)
				&& CommonUtilities.isValueNull(quantity)) {
			return true;
		} else if (!CommonUtilities.isValueNull(price)
				&& CommonUtilities.isValueNull(quantity)) {
			return false;
		} else if (CommonUtilities.isValueNull(price)
				&& !CommonUtilities.isValueNull(quantity)) {
			return false;
				...
				&& !PriceGridUtil.isValidQuantity(quantity)) {
			return false;
		}
		return true;
	}
	...
	protected static String getPriceCriteriaCombinationString(
			String criteriaCode1, String criteria1, String criteriaCode2,
			String criteria2) {

		return criteriaCode1 + "_" + criteria1 + "_" + criteriaCode2 + "_"
				+ criteria2;
	}
	...
	protected static int findNumberOfPGridAvailable(String[] prices) {
		int count = 0;
		for (int i = 0; i < prices.length; i++) {
			count = CommonUtilities.isElementsAreNull(prices[i], "$") ? count
					: ++count;
		}
		return count;
	}
	 ...
	 * @return a collection of upcharge prices, otherwise empty list if the
	 *         given collection is null or the collection dosen't have any
	 *         ucharges
	 */
	public List<PriceGrids> getUpchargePrices(PriceGrids[] exisistingPGs,
		...
		if (exisistingPGs != null) {
			for (PriceGrids pGrid : exisistingPGs) {
				if (pGrid != null
						&& !CommonUtilities.getBooleanValueFromString(pGrid
								.getIsBasePrice())) {
					if (pGrid.getPricingItems() != null
							&& pGrid.getPricingItems().length > 0) {
						PricingItems[] pricingItems = getValidPricingItem(
								pGrid.getPricingItems(), criteriaSetValueIds)
								.toArray(new PricingItems[0]);
	...
	private List<PricingItems> getValidPricingItem(PricingItems[] pricingItems,
			List<String> criteriaSetValueIds) {
	...
	public Set<String> getAllCriteriaSetValueIds(
			ProductConfigurations productConfigurations) {
		Set<String> criteriaSetValueIds = new HashSet<String>();
		if (productConfigurations != null
				&& productConfigurations.getProductCriteriaSets() != null
				&& productConfigurations.getProductCriteriaSets().length > 0) {
			for (ProductCriteriaSets prodCriteriaSet : productConfigurations
					.getProductCriteriaSets()) {
				if (prodCriteriaSet != null) {
					for (CriteriaSetValues criteriaSetValue : prodCriteriaSet
	...
	protected boolean processPriceGridForSimplifiedTemplate(
			String priceCriteria, String criteriaCode) {
		HashMap<String, String> tempMap = new HashMap<>();
		String criteriaSetValueId = "";

		if ("specialCriterias".contains(criteriaCode)) { // TODO : Change
															// ***specialCriterias***
															// to actual codes
			// checking the pricing item contains an element for that match
			if (tempMap.containsKey(criteriaCode + "__" + criteriaSetValueId)) {
				String value = tempMap.get(criteriaCode + "__"
						+ criteriaSetValueId);
	...
	public static boolean isPricesEqual(Prices p1, Prices p2) {
		if (!String.valueOf(p1.getListPrice()).trim()
				.equalsIgnoreCase(String.valueOf(p2.getListPrice()).trim())) {
			return false;
		}
		if (!String.valueOf(p1.getQuantity()).trim()
				.equalsIgnoreCase(String.valueOf(p2.getQuantity()).trim())) {
			return false;
		}
		if (p1.getDiscountRate() != null
				&& p2.getDiscountRate() != null
				&& !String
						.valueOf(p1.getDiscountRate().getCode())
						.trim()
						.equalsIgnoreCase(
								String.valueOf(p2.getDiscountRate().getCode()
										.trim()))) {
			return false;
		}
		return true;
	...
	protected boolean isCriteriaCodeValidForPriceGrid(String criteriaCode,
			boolean isBasePrice) {
		if (CommonUtilities.isValueNull(criteriaCode)) {
			return true;
		}
		if (isBasePrice) {
			return ApplicationConstants.BASE_PRICE_CRITERIA_SET
					.contains(criteriaCode.trim().toUpperCase());
		} else {
			return ApplicationConstants.UPCHARGE_PRICE_CRITERIA_SET
					.contains(criteriaCode.trim().toUpperCase());
		}
	}
	...
	protected Set<String> getOptionGroupPriceCriterias(String[] criteria1Array,
			String[] criteria2Array) {
		...
		try {
			for (String criteria : criteria1Array) {
				if (!CommonUtilities.isValueNull(criteria)) {
					...
					i++;
					String criteriaCode = criteria.trim().substring(0,
							criteria.indexOf(":"));
					if (isOptionGroup(criteriaCode)) {
						if (CommonUtilities.countSpecifiedChar(criteria, ':') > 1) {
							String valueToPreserve = criteria.trim().substring(
									0,
									CommonUtilities.indexOfElement(criteria,
											':', 2));
	...
	protected boolean isOptionCriteriaAlreadyConfigured(
			String upchargeCriteria, Set<String> optionGroupCriteriaSet) {
		if (!CommonUtilities.isValueNull(upchargeCriteria)) {
			String criteriaCode = upchargeCriteria.trim().substring(0,
					upchargeCriteria.indexOf(":"));
			if (isOptionGroup(criteriaCode)) {
				if (CommonUtilities.countSpecifiedChar(upchargeCriteria, ':') > 1) {
					String valueToPreserve = upchargeCriteria.trim().substring(
							0,
							CommonUtilities.indexOfElement(upchargeCriteria,
									':', 2));
	...
	/**
	 * Some of the Product Criteria need overcome the rules of base price Grid
	 * creation. This rule relaxtion determined by Criteria codes
	 * 
	 * @param criteriaPaticipator
	 * @param criteriaCodeToCheck
	 * @return
	 */
	private static boolean relaxBasePriceRule(String criteriaPaticipator,
			String criteriaCodeToCheck) {
		if (PriceGridUtil.isCriteriaInPRSGroup(criteriaPaticipator)
				&& PriceGridUtil.isCriteriaInPRSGroup(criteriaCodeToCheck)) {
			return true;
		}
		return false;
	}
	...
	public com.asi.ext.api.service.model.Product setProductWithPriceGrids(
		...
		if (null != pricegridsList && pricegridsList.size() > 0) {
			...
			for (PriceGrid radarPriceGrid : pricegridsList) {
				...
				currentPriceGrid
						.setIsBasePrice(radarPriceGrid.getIsBasePrice());
				currentPriceGrid.setIsQUR(radarPriceGrid.getIsQUR());
				currentPriceGrid
						.setDescription(radarPriceGrid.getDescription());
				currentPriceGrid.setPriceIncludes(radarPriceGrid
						.getPriceIncludes());
				currentPriceGrid.setSequence(radarPriceGrid
						.getDisplaySequence());
				//
				if (null == currenciesWSList) {
					currenciesWSList = lookupRestTemplate
							.getForObject(
									RestAPIProperties
											.get(ApplicationConstants.CURRENCIES_LOOKUP_URL),
									LinkedList.class);
				}
				currentPriceGrid.setCurrency(jsonProcessorObj
						.checkCurrencyValueKeyPair(productDetail
								.getExternalProductId().trim(),
								currenciesWSList, radarPriceGrid.getCurrency()
										.getCode()));
				// Product Number - TBD
				currentPriceGrid.setProductNumber(getProductNumber(
						radarPriceGrid.getPricingItems(),
				...
				if (null != pricesList && pricesList.size() > 0) {
					...
					for (com.asi.service.product.client.vo.Price currentPrice : pricesList) {
						...
						currentPrices.setSequence(currentPrice
								.getSequenceNumber());
						currentPrices.setQty(currentPrice.getQuantity());
						currentPrices.setListPrice(String.valueOf(currentPrice
								.getListPrice()));
						if (isValidDiscountCode(currentPrice.getDiscountRate()
								.getIndustryDiscountCode()))
							currentPrices.setDiscountCode(currentPrice
									.getDiscountRate()
									.getIndustryDiscountCode());
						if (null != currentPrice.getPriceUnit()) {
							currentPriceUnit = new com.asi.ext.api.service.model.PriceUnit();
							if (null != currentPrice.getItemsPerUnit()
									&& currentPrice.getItemsPerUnit() > 0)
								currentPriceUnit
										.setItemsPerUnit(String
												.valueOf(currentPrice
														.getItemsPerUnit()));

							currentPriceUnit
									.setName((null != currentPrice
											.getPriceUnit().getDisplayName() && !currentPrice
											.getPriceUnit().getDisplayName()
											.isEmpty()) ? currentPrice
											.getPriceUnit().getDisplayName()
											: null);
							if (ProductDataStore.isOtherPriceUnit(currentPrice
									.getPriceUnit().getDisplayName())) {
								currentPriceUnit.setPriceUnitName(currentPrice
										.getPriceUnitName());
							}
					...
					// currentPriceGrid.setPriceConfigurations(setPriceGridWithItsPriceCriteria(radarPriceGrid,
					// productDetail));
				}

				currentPriceGrid
						.setPriceConfigurations(setPriceGridWithItsPriceCriteria(
								radarPriceGrid, productDetail));
	...
	private String getProductNumber(List<PricingItem> pricingItems,
				...
				for (ProductNumber currentProductNumber : productNumbers) {
					for (ProductNumberConfiguration currentProductNumberConfig : currentProductNumber
						...
						if (currentPricingItem.getCriteriaSetValueId().equals(
								String.valueOf(currentProductNumberConfig
										.getCriteriaSetValueId()))) {
							finalProductNumber = currentProductNumber
									.getValue();
							break;
						}
	...
	private boolean isValidDiscountCode(String code) {
		DiscountRate currentDiscountRate = ProductDataStore.getDiscountRate(
				code, true);
		if (null != currentDiscountRate
				&& currentDiscountRate.getIndustryDiscountCode()
	...
	private List<PriceConfiguration> setPriceGridWithItsPriceCriteria(
		...
		if (currentPriceGrid != null) {
			...
			pricingConfigurations = pricesParser.getPricingConfigurations(
