...
public class ProductSpecSampleProcessor extends SimpleCriteriaProcessor { // SPECIAL Processing needed
    ...
    public ProductCriteriaSets getCriteriaSetForMultiple(String specSample, String prodSample, Product existingProduct,
        ...
        if (CommonUtilities.isUpdateNeeded(prodSample)) {
            if (!CommonUtilities.isValueNull(prodSample)) {
                if (productSampleCriteriaSetValue == null 
                        || !String.valueOf(productSampleCriteriaSetValue.getCriteriaValueDetail()).toUpperCase().equalsIgnoreCase(prodSample.trim())) {
        ...
        if (CommonUtilities.isUpdateNeeded(specSample)) {
            if (!CommonUtilities.isValueNull(specSample)) {
                ...
                if (specSampleCriteriaSetValue == null 
                        || !String.valueOf(specSampleCriteriaSetValue.getCriteriaValueDetail()).toUpperCase().equalsIgnoreCase(specSample.trim())) {
                    // Existing criteria value matched
        ...
        if (productSampleCriteriaSetValue != null) {
            updateReferenceTable(existingProduct.getExternalProductId(), ApplicationConstants.CONST_PRODUCT_SAMPLE_CRITERIA_CODE, productSampleCriteriaSetValue.getCriteriaValueDetail(),
                    productSampleCriteriaSetValue);
            finalValues.add(productSampleCriteriaSetValue);
        }
        if (specSampleCriteriaSetValue != null) {
            updateReferenceTable(existingProduct.getExternalProductId(), ApplicationConstants.CONST_PRODUCT_SAMPLE_CRITERIA_CODE, specSampleCriteriaSetValue.getCriteriaValueDetail(),
                    specSampleCriteriaSetValue);
            finalValues.add(specSampleCriteriaSetValue);
        }
        
        matchedCriteriaSet.setCriteriaSetValues(finalValues.toArray(new CriteriaSetValues[0]));
