...
/**
 * This class is used store elements for product transformation purpose and many static functions which required for better
 * processing.
 * 
 * @author Rahul K
 * 
...
public class ProductDataStore {
     ...
     * this collection will be processed by batch processor and many tools for manipulating batch errors.
     * 
     * <B>ExternalId </B> and <B>BatchErrorCode</B> are cannot be null, if any of these two values are empty then function will not
     * add error message to batch log
     * 
     * @param externalPrdId
    ...
    public static String findCategoryCode(String category) { // throws VelocityException {
        if (categoryCodeLookupTable == null || categoryCodeLookupTable.isEmpty()) {
            // Create Category Lookup table
            try {
                String categoryResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.PRODUCT_CATEGORIES_LOOKUP_URL));
                if (categoryResponse == null || categoryResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for Categories
                    // throw new VelocityException("Unable to get response from Category API", null);
    ...
    public static String getSetCodeValueIdForProductOrigin(String origin) {
        if (productOriginsLookupTable == null || productOriginsLookupTable.isEmpty()) {
            // Create Category Lookup table
            try {
                String productOriginResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.ORIGIN_LOOKUP_URL));
                if (productOriginResponse == null || productOriginResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for Origin
                    // throw new VelocityException("Unable to get response from Origin API", null);
    ...
    public static String getSetCodeValueIdForProductShape(String shape) {
        if (productShapesLookupTable == null || productShapesLookupTable.isEmpty()) {
            ...
            try {
                String productShapesResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.PRODUCT_SHAPES_LOOKUP_URL));
                if (productShapesResponse == null || productShapesResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for Origin
                    // throw new VelocityException("Unable to get response from Shapes API", null);
        ...
        if (productThemesLookupTable == null || productThemesLookupTable.isEmpty()) {
            ...
            try {
                ...
                if (productThemesResponse == null || productThemesResponse.isEmpty()) {
                    // throw new VelocityException("Unable to get response from themes API", null);
                    // Report error to API that we are not able to fetch data for themes
    ...
    public static String getSetCodeValueIdForProductTradeName(String tradeName) {
        try {
            ...
            if (productTradeNameResponse == null || productTradeNameResponse.isEmpty()) {
                // Report error to API that we are not able to fetch data for Origin
                // throw new VelocityException("Unable to get response from tradename API", null);
    ...
    public static String getSetCodeValueIdForProductPackage(String packaging) {
        if (productPackagesLookupTable == null || productPackagesLookupTable.isEmpty()) {
            ...
            try {
                String productPackagesResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.PACKAGING_LOOKUP));
                if (productPackagesResponse == null || productPackagesResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for packages
                    // throw new VelocityException("Unable to get response from packages API", null);
    ...
    public static String getSetCodeValueIdForAdditionalColor(String additionalColor) {
        if (additionalColorLookupTable == null || additionalColorLookupTable.isEmpty()) {
            ...
            try {
                String additionalColorResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.ADDITIONAL_COLOR_LOOKUP));
                if (additionalColorResponse == null || additionalColorResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for additional color
                    // throw new VelocityException("Unable to get response from additional color API", null);
    ...
    public static String getSetCodeValueIdForAdditionalLocation(String trim) {
        if (additionalLocationLookupTable == null || additionalLocationLookupTable.isEmpty()) {
            ...
            try {
                String additionalLocationResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.ADDITIONAL_LOCATION_LOOKUP));
                if (additionalLocationResponse == null || additionalLocationResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for additional location
                    // throw new VelocityException("Unable to get response from additional location API", null);
    ...
    public static String getSetCodeValueIdForImprintColor(String imprintColor) {
        if (imprintColorLookupTable == null || imprintColorLookupTable.isEmpty()) {
            ...
            try {
                String imprintColorResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.IMPRINT_COLOR_LOOKUP));
                if (imprintColorResponse == null || imprintColorResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for imprint color
                    // throw new VelocityException("Unable to get response from imprint color API", null);
    ...
    public static String getComplianceCertId(String compliance) {
        ...
        if (complianceCertLookupTable == null || complianceCertLookupTable.isEmpty()) {
            ...
            try {
                String complianceCertResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.PRODUCT_COMPLIANCECERTS_LOOKUP));
                if (complianceCertResponse == null || complianceCertResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for Compliance Certificate
                    // throw new VelocityException("Unable to get response from  Compliance Certificate API", null);
    ...
    public static String getSelectedSafetyWarningCode(String safetywarning) {
        if (safetyWarningLookupTable == null || safetyWarningLookupTable.isEmpty()) {
            ...
            try {
                String safetyWarningResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.SAFETY_WARNINGS_LOOKUP));
                if (safetyWarningResponse == null || safetyWarningResponse.isEmpty()) {
                    // Report error to API that we are not able to fetch data for Selected Safety warning
                    // throw new VelocityException("Unable to get response from  Selected Safety warning API", null);
    ...
    public static String getSetCodeValueIdForProdSpecSample(String value) {
        if (prodSpecSampleLookupTable == null || prodSpecSampleLookupTable.isEmpty()) {
            try {
                String prodSpecSampleJson = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.ADDITIONAL_COLOR_LOOKUP));
                if (prodSpecSampleJson == null || prodSpecSampleJson.isEmpty()) {
                    // Report error to API that we are not able to fetch data for Product Spec Sample
                    // throw new VelocityException("Unable to get response from  Product Spec Sample API", null);
    ...
    public static String getSetCodeValueIdForProductionTime(String value) {
        if (productionTimeLookupTable == null || productionTimeLookupTable.isEmpty()) {
            try {
                String productionTimeResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
    ...
    public static String getSetCodeValueIdForSameDayService(String value) {
        if (sameDayRushServiceLookupTable == null || sameDayRushServiceLookupTable.isEmpty()) {
            try {
                String sameDayRushResponse = JersyClientGet.getLookupsResponse(RestAPIProperties
                        .get(ApplicationConstants.RUSH_TIME_LOOKUP));
    ...
    /**
     * This method will search for the particular item in the Lookup table of given criteria code
     * using the SetCodeValueId. Its called reverse lookup and used to decode SetCodeValue of a CriteriaSetValue element.
     * 
     * TODO : Not Completed
     * 
